...
select cl.num_cadastro, cl.nome, fl.titulo, max( d.data_fabricacao) as data
from cliente cl, filme fl, dvd d, locacao l 
where cl.num_cadastro = l.clientenum_cadastro
	and l.dvdnum = d.num
	and d.filmeid = fl.id
group by d.data_fabricacao, cl.num_cadastro, cl.nome, fl.titulo, d.data_fabricacao
order by d.data_fabricacao desc

select c.num_cadastro, c.nome, convert(char(10), l.data_locacao, 103) as data ,count (l.dvdnum) as qtde
from cliente c, locacao l
where l.clientenum_cadastro = c.num_cadastro
group by c.num_cadastro, c.nome, l.data_locacao
order by c.nome	


select cliente.nome, cliente.num_cadastro, locacao.data_locacao,sum( locacao.valor) as total 
from cliente inner join locacao
on cliente.num_cadastro = locacao.clientenum_cadastro
group by cliente.nome, cliente.num_cadastro, locacao.data_locacao
